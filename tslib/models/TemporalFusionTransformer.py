...
def get_known_len(embed_type, freq):
    ...
    if embed_type != 'timeF':
        ...
        freq_map = {'h': 4, 't': 5, 's': 6,
                    'm': 1, 'a': 1, 'w': 2, 'd': 3, 'b': 3,
                    'min': 5}  # 'min' 等价于 't'（分钟级）
        return freq_map.get(freq, 4)  # 默认返回 4，避免 KeyError
...
class TFTEmbedding(nn.Module):
    def __init__(self, configs):
        ...
        # 支持 custom 数据集：动态获取 static/observed 位置
        if configs.data in datatype_dict:
            ...
            # custom 数据集：所有变量都作为 observed，无 static 特征
            self.static_pos = []
            self.observed_pos = list(range(configs.enc_in))
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Number of variables - 支持 custom 数据集
        if configs.data in datatype_dict:
            ...
            # custom 数据集：所有变量都作为 observed，无 static 特征
            self.static_len = 0
            self.observed_len = configs.enc_in
