...
class Dataset_Custom(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
        ...
        if size == None:
            ...
            self.label_len = size[1]
            self.pred_len = size[2]
        # 标准预测模式：encoder 仅使用历史 seq_len；预测从 s_end 开始，额外需要 pred_len 个未来点。
        if self.label_len > self.seq_len:
            raise ValueError(f"label_len({self.label_len}) must be <= seq_len({self.seq_len})")
        # init
        assert flag in ['train', 'test', 'val']
        type_map = {'train': 0, 'val': 1, 'test': 2}
        self.set_type = type_map[flag]
    ...
    def __read_data__(self):
        ...
        train_ratio = float(getattr(self.args, 'train_ratio', 0.7))
        val_ratio = float(getattr(self.args, 'val_ratio', 0.1))
        test_ratio = float(getattr(self.args, 'test_ratio', 0.2))
        split_sum = train_ratio + val_ratio + test_ratio
        if train_ratio <= 0 or test_ratio <= 0 or val_ratio < 0 or abs(split_sum - 1.0) > 1e-6:
            raise ValueError(
                f"Invalid Dataset_Custom split ratios: train={train_ratio}, val={val_ratio}, test={test_ratio}."
            )

        n_total = len(df_raw)
        if val_ratio == 0.0:
            # 复训模式（train+val）：固定让验证集计数为 0，避免浮点/取整导致“名义 0 实际 1”。
            num_test = int(round(n_total * test_ratio))
            num_test = min(max(num_test, 1), n_total - 1)
            num_train = n_total - num_test
            num_vali = 0
        else:
        ...
        if num_train <= 0 or num_test <= 0 or num_vali < 0:
            raise ValueError(
                f"Dataset_Custom split size invalid: train={num_train}, val={num_vali}, test={num_test}, n_total={n_total}."
            )

        if num_vali == 0:
            # retrain 模式：训练区间使用 train+val 全段；验证区间与训练尾部重叠（仅用于保持训练循环兼容）。
            val_window = self.seq_len + self.pred_len
            val_start = max(0, num_train - val_window)
            border1s = [0, val_start, n_total - num_test - self.seq_len]
            border2s = [num_train, num_train, n_total]
        else:
            train_start = 0
            val_end = num_train + num_vali
            val_start = max(0, num_train - self.seq_len)
            # 验证集过短时向前扩展窗口，避免 __len__ 为负
            min_val_window = self.seq_len + self.pred_len
            if (val_end - val_start) < min_val_window:
                val_start = max(0, val_end - min_val_window)
            test_start = max(0, n_total - num_test - self.seq_len)
            border1s = [train_start, val_start, test_start]
            border2s = [num_train, val_end, n_total]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]
    ...
    def __getitem__(self, index):
        ...
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
        if getattr(self.args, 'loss_time_weighted', False) and self.set_type == 0:
            return seq_x, seq_y, seq_x_mark, seq_y_mark, index
        return seq_x, seq_y, seq_x_mark, seq_y_mark

    def __len__(self):
        # 标准模式需要额外 pred_len 的未来点
        return len(self.data_x) - self.seq_len - self.pred_len + 1
