...
当前两个模型都采用 **Crossformer 多变量时间序列模型**。可以把它理解为：模型不只看某一个点位过去的高低，还会同时比较许多点位在过去 20 分钟内的先后变化和相互关系，再给出未来 10 分钟的整段趋势。这样做适合焚烧炉这类“一个现象往往同时受多处工况影响”的场景，例如配风、炉温、料层和蒸汽流量会共同影响燃烧状态。

选择这类模型的原因是：它能够在较长的历史窗口内利用多点位信息，直接输出连续的未来轨迹；同时训练、ONNX 导出和后端离线推理链已经完成验证。它不是控制器，也不会自行判断应调什么变量；它的职责只是尽可能早、尽可能稳定地提供工况变化依据。

系统采用两个模型，而不是用一个模型包揽全部目标，主要有三点考虑：

1. **关注对象不同。** 模型 1 面向燃烧和蒸汽工况；模型 2 面向烟气净化和排放。两组目标的业务含义、主要影响点位和后续使用方式不同。
2. **变化节奏不同。** 炉膛与料层变化通常更适合十秒级观察；烟气排放数据保留逐秒信息，便于检查较快的波动。两者虽然都预测未来 10 分钟，但不宜强行用同一套时间网格而不做验证。
3. **便于分别优化和验收。** 某一侧效果不足时，可以只调整对应模型的数据、特征或训练参数，不会把另一侧已经稳定的预测一起改动；上线前也能按各自的五个目标独立验收。

这是一种当前数据条件下更稳妥的拆分方式，并不代表两个模型永远不能统一。后续应以相同数据切分上的对比结果为依据，评估统一时间步长或单模型是否真能提升关键工况下的预测可靠性，再决定是否新建版本替换。
